  ...
  useEffect(() => {
    api.get(`/contributions?page=${page}&page_size=${pageSize}&ordering=-created_at`).then((res: any) => {
      ...
      let contribs = res.results.map((contr: any) => {
        ...
        return {
          "Título": contr.title,
          "Descrição": contr.description.length > 20 ? contr.description.substring(0, 20) + '...' : contr.description,
          ...
          "Executado em": date.toLocaleString(
            'pt-BR', {
              day: '2-digit',
              month: '2-digit',
              year: 'numeric',
  ...
  return (
    <>
      <PaginatedTable
        ...
        headers={["Título", "Descrição", "Desconto%", "Vlr R$", "Qtd", "Total R$", "Fornecedor", "Objetivo", "Executado em"]}
        data={contributions}
